...
async function main() {
  ...
  const [fleetMgr, driver, safetyOfficer, financialAnalyst] = await Promise.all([
    prisma.user.upsert({
  ...
  console.log('✅ Users seeded:', [fleetMgr, driver, safetyOfficer, financialAnalyst].map(u => u.email));
